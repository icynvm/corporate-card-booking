        ...
        return (
            <div className="space-y-8 relative max-w-5xl mx-auto">
                <GlassCard className="text-center py-12">
                    ...
                    <p className="text-gray-400 dark:text-gray-500 text-sm">Loading request details...</p>
                </GlassCard>
            </div>
        )
    }
    ...
    return (
        <div className="space-y-6 relative max-w-5xl mx-auto pb-10">
            ...
            <div className="flex items-center gap-3 mb-2">
                <button onClick={() => router.back()} className="p-2 -ml-2 rounded-full hover:bg-gray-100 dark:bg-gray-800/80 transition-colors text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:text-gray-300">
            ...
            <GlassCard className="!p-0 overflow-hidden shadow-sm">
                {/* ─── Header & Actions ─── */}
                <div className="p-5 flex flex-col md:flex-row md:items-start justify-between gap-5 bg-white dark:bg-gray-800">
                    <div className="flex-1 space-y-4">
                        <div className="flex items-center gap-3 flex-wrap">
                            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${STATUS_COLORS[request.status] || "bg-gray-100 dark:bg-gray-800/80 text-gray-600 dark:text-gray-300"}`}>
                        ...
                        <div>
                            <h3 className="font-bold text-gray-800 dark:text-gray-100 text-xl break-words">{request.project_name || "N/A"}</h3>
                        </div>
                    ...
                    <div className="p-5 space-y-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                                <div className="px-5 py-3 bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100 flex items-center gap-2">
                                    <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Requester Information</h4>
                                </div>
                                <div className="p-5 space-y-3 text-sm">
                                    <div className="flex flex-wrap justify-between gap-1">
                                        <span className="text-gray-400 dark:text-gray-500 text-xs">Name</span>
                                        <span className="font-medium text-gray-800 dark:text-gray-100 text-sm break-all text-right">{request.full_name || request.profiles?.name || "N/A"}</span>
                                    </div>
                                    <div className="flex flex-wrap justify-between gap-1">
                                        <span className="text-gray-400 dark:text-gray-500 text-xs">Department</span>
                                        <span className="font-medium text-gray-800 dark:text-gray-100 text-sm break-all text-right">{request.department || request.profiles?.department || "N/A"}</span>
                                    </div>
                                    <div className="flex flex-wrap justify-between gap-1">
                                        <span className="text-gray-400 dark:text-gray-500 text-xs">Email</span>
                                        <span className="font-medium text-gray-800 dark:text-gray-100 text-sm break-all text-right">{request.email || "N/A"}</span>
                                    </div>
                                    <div className="flex flex-wrap justify-between gap-1">
                                        <span className="text-gray-400 dark:text-gray-500 text-xs">Contact</span>
                                        <span className="font-medium text-gray-800 dark:text-gray-100 text-sm break-all text-right">{request.contact_no || "N/A"}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Project Details */}
                            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                                <div className="px-5 py-3 bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100">
                                    <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Project Details</h4>
                                </div>
                                <div className="p-5 space-y-3 text-sm">
                                    <div>
                                        <span className="text-gray-400 dark:text-gray-500 text-xs block mb-1">Objective</span>
                                        <p className="font-medium text-gray-800 dark:text-gray-100 text-sm break-words">{request.objective}</p>
                                    </div>
                                    <div className="pt-2 border-t border-gray-50 grid grid-cols-2 gap-4">
                                        <div>
                                            <span className="text-gray-400 dark:text-gray-500 text-xs block mb-1">Start Date</span>
                                            <p className="font-semibold text-gray-800 dark:text-gray-100 text-sm">{fmtDate(request.start_date)}</p>
                                        </div>
                                        <div>
                                            <span className="text-gray-400 dark:text-gray-500 text-xs block mb-1">End Date</span>
                                            <p className="font-semibold text-gray-800 dark:text-gray-100 text-sm">{fmtDate(request.end_date)}</p>
                                        </div>
                                    </div>
                                    {(request.booking_date || request.effective_date) && (
                                        <div className="grid grid-cols-2 gap-4 pt-2 border-t border-gray-50">
                                            <div>
                                                <span className="text-gray-400 dark:text-gray-500 text-xs block mb-1">Booking Date</span>
                                                <p className="font-semibold text-gray-800 dark:text-gray-100 text-sm">{fmtDate(request.booking_date)}</p>
                                            </div>
                                            <div>
                        ...
                        {request.approval_notes && (
                            <div className="bg-white dark:bg-gray-800 rounded-xl border border-amber-100 shadow-sm overflow-hidden">
                                ...
                                <div className="p-5">
                                    <p className="text-sm text-gray-800 dark:text-gray-100 whitespace-pre-wrap break-words leading-relaxed">{request.approval_notes}</p>
                                </div>
                            </div>
                        )}
                        ...
                        {request.promotional_channels && request.promotional_channels.length > 0 && (
                            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                                <div className="px-5 py-3 bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100">
                                    <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Promotional Channels</h4>
                                </div>
                                <div className="p-5">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {request.promotional_channels.map((chan: { channel: string; mediaAccountEmail: string; accessList: string }, idx: number) => (
                                            <div key={idx} className="bg-gray-50 dark:bg-gray-900/50/80 p-4 rounded-xl border border-gray-100 shadow-sm text-xs transition-colors hover:bg-gray-50 dark:bg-gray-900/50">
                                                ...
                                                <div className="space-y-2">
                                                    <div>
                                                        <span className="text-gray-400 dark:text-gray-500 uppercase text-[10px] block font-semibold tracking-wider">Account</span>
                                                        <span className="text-gray-800 dark:text-gray-100 font-medium break-all">{chan.mediaAccountEmail || "N/A"}</span>
                                                    </div>
                                                    <div>
                                                        <span className="text-gray-400 dark:text-gray-500 uppercase text-[10px] block font-semibold tracking-wider">Access</span>
                                                        <span className="text-gray-800 dark:text-gray-100 font-medium break-all">{chan.accessList || "N/A"}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                        ...
                        {request.approval_file_url && (
                            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                                <div className="px-5 py-3 bg-gray-50 dark:bg-gray-900/50 border-b border-gray-100">
                                    <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wider">Signed Approval Document</h4>
                                </div>
                                <div className="p-5 flex items-center gap-4 flex-wrap">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-bold text-gray-800 dark:text-gray-100 break-words mb-0.5">
                                            {request.approval_file_url.startsWith("data-ref:") ? request.approval_file_url.replace("data-ref:", "") : "Approval Document"}
                                        </p>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500">Official signed copy</p>
                                    </div>

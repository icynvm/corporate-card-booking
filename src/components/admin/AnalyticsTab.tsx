...
export function AnalyticsTab({ requests }: AnalyticsTabProps) {
    ...
    // Analytics calculations on filtered data
    const stats = useMemo(() => {
        let totalSpent = 0;
        let pendingAmount = 0;
        ...
        filtered.forEach((req) => {
            ...
            // Monthly trend (using request start_date)
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div>
                            ...
                            <p className="text-xs text-gray-400 mt-0.5">{rangeLabel} · <span className="font-semibold text-brand-600">{filtered.length}</span> requests in range</p>
                        </div>
                        {/* Quick presets */}
                            ...
                            {PRESETS.map((p) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${isActive ? "bg-brand-500 text-white shadow-md shadow-brand-200" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
                                    >
                                        {p.label}
                                    </button>
                                );
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 sm:items-end">
                        ...
                        <span className="hidden sm:block text-gray-300 font-bold pb-2.5">→</span>
                        <div className="flex-1">
                ...
                <GlassCard className="!p-5">
                    ...
                    <div className="space-y-5">
                        {stats.topProjects.length === 0 ? (
                            <p className="text-gray-400 text-sm text-center py-6">No project data in selected range</p>
                        ) : (

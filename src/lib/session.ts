...
// Simple token: base64(JSON({profileId, email, role, name, department, exp}))
// In production, use JWT. This is sufficient for dev/internal.
...
export async function sendOTPEmail(email: string, code: string, name?: string) {
    ...
    if (!resendKey || resendKey === "re_xxxxxxxxxxxx" || resendKey === "re_dummy_key_for_build") {
